...
| GPU, MSI → forced legacy line | Nothing we would bet on: mean ISR 19.2 → 25.4 µs in the fresh-boot pair, but MSI-mode runs alone ranged 18.9–25.9 µs | ISR + DPC per interrupt (38 vs 42 µs); 0 missed frames in both modes (the MSI-mode frame data comes from a long-uptime session: PresentMon captured nothing in the fresh-boot one) |
| USB xHCI with a fast mouse (~7,500 interrupts/s), MSI → forced legacy line | Mean ISR 0.2 → 1.1 µs | Mean DPC 5.6 µs both ways; **+0.9 µs per mouse report** in total (frame data exists for the legacy run only: 1 missed frame in 10,788) |
